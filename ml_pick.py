        ...
        for f in fixtures:
            try:
                ...
                try:
                    ...
                    pass
                results.append({'fixture_id': fid, 'league_id': lid, 'league_name': ctx.get('league_name'), 'country': ctx.get('country'), 'home': ctx.get('home'), 'away': ctx.get('away'), 'minute': m, 'score': '%d-%d' % (sh, sa), 'picks': picks[:6], 'ml_hints': ml_hints})
            except Exception:
                continue
        results.sort(key=lambda r: -(r['picks'][0]['edge_pct'] if r['picks'] else 0))
...
<style>body{font-family:sans-serif;margin:0;padding:16px;background:#0d1117;color:#e6edf3}h1{margin:0 0 4px;font-size:22px}.muted{color:#8b949e;font-size:13px}.controls{display:flex;flex-wrap:wrap;gap:12px;margin:16px 0;padding:12px;background:#161b22;border-radius:8px;border:1px solid #30363d}.controls label{font-size:12px;color:#8b949e;display:block}.controls input{width:100px;padding:6px 8px;background:#0d1117;color:#e6edf3;border:1px solid #30363d;border-radius:4px}.btn{padding:8px 16px;background:#238636;color:#fff;border:none;border-radius:4px;cursor:pointer}.fixture{margin:12px 0;padding:12px;background:#161b22;border:1px solid #30363d;border-radius:8px}.fix-head{display:flex;justify-content:space-between;border-bottom:1px solid #30363d;padding-bottom:8px;margin-bottom:8px}.fix-teams{font-weight:600;font-size:15px}.fix-meta{color:#8b949e;font-size:12px}.score{color:#f0883e;font-weight:600}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;padding:6px 8px;color:#8b949e;font-size:11px;text-transform:uppercase}td{padding:6px 8px;border-top:1px solid #21262d}.pos{color:#3fb950;font-weight:600}.market{font-weight:600}.stake{color:#f0883e;font-weight:600}.ml-hint{margin-top:8px;padding:6px 8px;background:#0d1117;border-radius:4px;display:flex;flex-wrap:wrap;align-items:center;gap:6px}.hint-label{color:#8b949e;font-size:11px;text-transform:uppercase;font-weight:600}.hint-badge{padding:3px 8px;border-radius:12px;font-size:12px;font-weight:600}.hint-hi{background:#1a3a1a;color:#3fb950;border:1px solid #238636}.hint-mid{background:#1a2a3a;color:#58a6ff;border:1px solid #1f6feb}.hint-lo{background:#2a2a1a;color:#d29922;border:1px solid #9e6a03}#status{padding:12px;background:#161b22;border-radius:8px;border:1px solid #30363d;margin-bottom:12px}.empty{padding:24px;text-align:center;color:#8b949e;background:#161b22;border-radius:8px;border:1px dashed #30363d}a{color:#58a6ff;text-decoration:none}.pred-box{margin-top:10px;padding:10px 16px;background:linear-gradient(90deg,#0a1e0a,#0d1117);border:1px solid #238636;border-radius:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}.pred-icon{font-size:18px}.pred-lbl{font-size:11px;text-transform:uppercase;color:#8b949e;font-weight:600;letter-spacing:.05em}.pred-mkt{color:#3fb950;font-size:15px;font-weight:700}.pred-pr{color:#f0883e;font-weight:600}.pred-qt{color:#58a6ff}.pred-ed{color:#3fb950;font-weight:700}.pred-sk{color:#f0883e;font-weight:700;font-size:15px}.pred-sep{color:#30363d;margin:0 2px}.pred-no{color:#8b949e;font-style:italic;font-size:13px}.kbtns{display:flex;flex-wrap:wrap;gap:5px;margin-top:4px}.kbtn{padding:5px 10px;background:#21262d;color:#e6edf3;border:1px solid #30363d;border-radius:6px;cursor:pointer;font-size:12px;transition:background .15s}.kbtn:hover{background:#30363d}.kbtn.act{background:#1f6feb;border-color:#388bfd;color:#fff;font-weight:600}.live-pred{margin-top:32px}.live-pred h2{font-size:17px;margin:0 0 12px;color:#e6edf3}.lp-card{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin-bottom:10px}.lp-title{font-weight:700;font-size:14px;margin-bottom:4px}.lp-score{color:#8b949e;font-size:12px;margin-bottom:8px}.lp-row{display:flex;align-items:center;gap:8px;margin:3px 0}.lp-lbl{font-size:12px;color:#c9d1d9;width:160px;flex-shrink:0}.lp-track{flex:1;height:8px;background:#21262d;border-radius:4px;overflow:hidden}.lp-fill{height:100%;border-radius:4px}.lp-pct{font-size:12px;color:#8b949e;width:36px;text-align:right}</style></head><body>
<a href="/">&larr; Home</a> &middot; <a href="/ml">ML Predictor</a> &middot; <a href="/ml-accuracy">Accuracy</a> &middot; <a href="/early-goals">&#x23F0; Early Goal</a>
...
<div><label>Frazione Kelly</label><div class="kbtns"><button class="kbtn act" data-v="0.25" onclick="setKly(this)">¼ Kelly</button><button class="kbtn" data-v="0.5" onclick="setKly(this)">½ Kelly</button><button class="kbtn" data-v="0.75" onclick="setKly(this)">¾ Kelly</button><button class="kbtn" data-v="1" onclick="setKly(this)">1×</button><button class="kbtn" data-v="1.5" onclick="setKly(this)">1.5×</button><button class="kbtn" data-v="2" onclick="setKly(this)">2×</button></div><input type="hidden" id="kly" value="0.25"></div>
...
</div><div id="status" class="muted">Premi "Aggiorna" per caricare</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:8px;align-items:start"><div><h3 style="font-size:15px;color:#8b949e;margin:0 0 10px;font-weight:400">Pick con edge</h3><div id="results"></div></div><div><h3 style="font-size:15px;color:#8b949e;margin:0 0 10px;font-weight:400">🔮 Live Predizioni</h3><div id="livepred"></div></div></div>
<script>function setKly(b){document.querySelectorAll('.kbtn').forEach(x=>x.classList.remove('act'));b.classList.add('act');document.getElementById('kly').value=b.dataset.v}async function loadPicks(){const c=document.getElementById('cap').value,k=document.getElementById('kly').value,e=document.getElementById('edm').value,s=document.getElementById('stm').value,st=document.getElementById('status'),r=document.getElementById('results');st.textContent='Caricamento...';r.innerHTML='';try{const x=await fetch('/api/ml-live-picks-all?capital='+c+'&kelly='+k+'&edge_min='+e+'&stake_max='+s,{cache:'no-store'}),j=await x.json();if(j.error){st.textContent='Errore: '+j.error;return}st.innerHTML='Live: <b>'+j.fixtures_total+'</b> fixture, <b>'+j.fixtures_with_picks+'</b> con pick. Capitale: <b>EUR '+j.params.capital+'</b>, Kelly <b>'+j.params.kelly_mult+'</b>, edge min <b>'+(j.params.edge_min*100).toFixed(1)+'%</b>';if(!j.fixtures||!j.fixtures.length){r.innerHTML='<div class="empty">Nessuna pick: niente partite live coperte da quote o edge < soglia.</div>';return}let h='';for(const f of j.fixtures){h+='<div class="fixture"><div class="fix-head"><div><div class="fix-teams">'+(f.home||'?')+' vs '+(f.away||'?')+'</div><div class="fix-meta">'+(f.country||'')+' &middot; '+(f.league_name||'')+'</div></div><div><span class="score">'+f.score+'</span> &middot; '+f.minute+"'</div></div>";h+='<table><thead><tr><th>Mercato</th><th>Prob</th><th>Quota fair</th><th>Quota bookie</th><th>Bookie</th><th>Edge</th><th>Stake</th><th>EUR</th></tr></thead><tbody>';for(const p of f.picks){h+='<tr><td class="market">'+p.market_label+'</td><td>'+(p.prob*100).toFixed(1)+'%</td><td>'+p.fair_quota+'</td><td>'+p.bookie_quota+'</td><td>'+p.bookie+'</td><td class="pos">+'+p.edge_pct+'%</td><td>'+p.stake_pct+'%</td><td class="stake">EUR '+p.stake_eur+'</td></tr>'}h+='</tbody></table>';// ML hints section
if(f.ml_hints&&f.ml_hints.length){h+='<div class="ml-hint"><span class="hint-label">🤖 ML consiglia:</span>';for(const mh of f.ml_hints){const cls=mh.prob>=70?'hint-hi':mh.prob>=55?'hint-mid':'hint-lo';h+='<span class="hint-badge '+cls+'">'+mh.label+' '+mh.prob+'%</span>'}h+='</div>'}
if(f.picks&&f.picks.length>0){const t=f.picks[0];h+='<div class="pred-box"><span class="pred-icon">&#127919;</span><span class="pred-lbl">Predizione &nbsp;</span><strong class="pred-mkt">'+t.market_label+'</strong><span class="pred-sep"> &middot; </span><span class="pred-pr">'+(t.prob*100).toFixed(1)+'% prob</span><span class="pred-sep"> &middot; </span><span class="pred-qt">Quota '+t.bookie_quota+'</span><span class="pred-sep"> &middot; </span><span class="pred-ed">+'+t.edge_pct+'% edge</span><span class="pred-sep"> &middot; </span><span class="pred-sk">EUR '+t.stake_eur+'</span></div>'}else if(f.ml_hints&&f.ml_hints.length>0){const mh=f.ml_hints[0];h+='<div class="pred-box"><span class="pred-icon">&#127919;</span><span class="pred-lbl">Predizione ML &nbsp;</span><strong class="pred-mkt">'+mh.label+'</strong><span class="pred-sep"> &middot; </span><span class="pred-pr">'+mh.prob+'% prob (no edge sufficiente per pick)</span></div>'}h+='</div>'}r.innerHTML=h;const lp=document.getElementById('livepred');let lh='';if(j.fixtures&&j.fixtures.length){for(const fx of j.fixtures){if(!fx.ml_hints||!fx.ml_hints.length)continue;lh+='<div class="lp-card"><div class="lp-title">'+fx.home+' vs '+fx.away+'</div><div class="lp-score">'+fx.league_name+' · '+fx.score+' ('+fx.minute+"')</div>";for(const mh of fx.ml_hints){const fc=mh.prob>=70?'#238636':mh.prob>=55?'#9e6a03':'#1f6feb';lh+='<div class="lp-row"><span class="lp-lbl">'+mh.label+'</span><div class="lp-track"><div class="lp-fill" style="width:'+mh.prob+'%;background:'+fc+'"></div></div><span class="lp-pct">'+mh.prob+'%</span></div>'}lh+='</div>'}}lp.innerHTML=lh||'<span class="muted">Nessuna predizione live disponibile.</span>'}catch(err){st.textContent='Errore di rete: '+err.message}}</script></body></html>"""
